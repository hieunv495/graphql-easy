...
const getCRUDCheckDefinitionsWithStaticDomain = (
  resource: string,
  staticDomain: string
  ...
  const getDef = (action: string) => ({
    beforeResolve: (role: string) => [[role, staticDomain, resource, action]],
  });
...
export default getCRUDCheckDefinitionsWithStaticDomain;
